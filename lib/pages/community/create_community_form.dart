...
class CreateCommunityForm extends ConsumerWidget {
  const CreateCommunityForm({super.key});
        ...
        SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildFieldTitle("Community Name", true),
              CustomTextField(
                controller: ref.read(communityControllerProvider.notifier).nameController,
                hintText: "Community Name",
              ),
              _buildFieldTitle("Community Description", true),
              CustomTextField(
                controller: ref.read(communityControllerProvider.notifier).descriptionController,
                hintText: "Community Description",
                maxLines: 4,
              ),
              _buildFieldTitle("Community Profile Image", false),
  ...
  InkWell _buildAddButton(WidgetRef ref, {bool isProfile = true}) {
    final image = isProfile
        ? ref.watch(communityControllerProvider).profileImage
        : ref.watch(communityControllerProvider).bannerImage;
    return InkWell(
      onTap: () {
        if (isProfile) {
          ref.read(communityControllerProvider.notifier).pickProfileImage();
        } else {
          ref.read(communityControllerProvider.notifier).pickBannerImage();
        }
      },
      child: image == null
          ? Container(
